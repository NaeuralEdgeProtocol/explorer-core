...
class CustomJsonEncoder {
  ...
  /// Set keys of Map in alphabetical/lexicographioc order
  /// Remove format float ie. 89.0 -> 89
...
class AixpVerifier {
  ...
  bool _checkIfVerificationNeeded(Map<String, dynamic> jsonData) {
    if (jsonData.containsKey('EE_SIGN') && jsonData.containsKey('EE_HASH')) {
      return true;
    }
    return false;
  }
  ...
  bool verifyMessage(Map<String, dynamic> data) {
    if (_checkIfVerificationNeeded(data)) {
      var decoded = {...data};
      var eeSign = urlSafeBase64ToBase64(decoded["EE_SIGN"]);
      var eeSender = decoded["EE_SENDER"] as String;
      ...
      String sender = urlSafeBase64ToBase64(eeSender
          .replaceFirst(Constant.ADDR_PREFIX[0], '')
          .replaceFirst(Constant.ADDR_PREFIX[1], ''));
